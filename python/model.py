...
#establishing the connection
    ...
    activitydf['day'] = pd.to_datetime(sleepdf['day']).dt.date
    ...
    if len(activitydf)%84 == 0: #3 months  0 - 77-84, 85-(161-168) 
        ...
        # sql = '''SELECT rmdn FROM recommendations WHERE rmdn regexp '^[1]+' AND id =%s ORDER BY timestamp DESC LIMIT 1'''
        # cursor.execute(sql, (user,))
        # data2 = cursor.fetchone()
        #  #gets the older recommendation for that week
        # sql = "UPDATE matrix1 SET rating = '%s' WHERE id=%s and recom=%s" #post the avgsteps to db
        # cursor.execute(sql, (v, user, data2))
        # conn.commit()
        ...
        # plt.show()
        avgpredstep = (np.sum(predactdf.predstep[len(predactdf)-7:len(predactdf)]) /7).round() #find average of the predicted step count 

        """CALCULATING PREDICTED AVERAGE STEPS  """
        if avgpredstep >=  5714:
            avgpredstep = str(avgpredstep).lstrip('[').rstrip(']')
            predmsgw = "Your predicted steps for next week is " + avgpredstep +". Keep it up!"
        elif 3428 <= avgpredstep < 5714:
            avgpredstep = str(avgpredstep).lstrip('[').rstrip(']')
            predmsgw = "Your predicted steps for next week is " + avgpredstep +". What can we change?"
        else:
            avgpredstep = str(avgpredstep).lstrip('[').rstrip(']')
            predmsgw = "Your predicted steps for next week is " + avgpredstep +". Let's see how we can improve this."
    ...
    if len(datadf)%84 == 0: #3 months 
        ...
        plt.plot(day, mood) 
        plt.xlabel("Days")
        plt.ylabel("0 Not Great :(     Mood      1 :) Great ")
    ...
    if len(sleepdf)%84 == 0: #3 months  0 - 77-84, 85-(161-168) 
        ...
        # plt.show()
        avgpredsle = (np.sum(predsledf.predsle[len(predsledf)-7:len(predsledf)]) /7).round()#find average of the predicted step count 
        #print(sleepdf['starttime'].dt.total_seconds().std())
        """CALCULATING PREDICTED AVERAGE QUALITY AND USING STARTTIME TO PRODUCE matrix."""
        if avgpredsle >= 85:
            avgpredsle = str(avgpredsle).lstrip('[').rstrip(']')
            predmsgs = "Your predicted sleep quality for next week is " + avgpredsle +"%. Keep it up!"
            count7 += 1 
        else:
            avgpredsle = str(avgpredsle).lstrip('[').rstrip(']')
            predmsgs = "Your predicted sleep quality for next week is " + avgpredsle +"%. What can we change?"
            #if standard dev :
            #   sleepmatrix = ''
